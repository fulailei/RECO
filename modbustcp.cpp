...
ModbusTCP::ModbusTCP(string ip, uint16_t port,int slave_id)
{
    setupMobus(ip,port, slave_id);
}

ModbusTCP::ModbusTCP(string ip,uint16_t port){
    setupMobus(ip,port);
}

ModbusTCP::ModbusTCP(string ip){
    setupMobus(ip);
}

void ModbusTCP::setupMobus(string ip, uint16_t port,int slave_id){
    host=ip;
    this->port=port;
    bConnected=false;
...
string ModbusTCP::error_info(string cs,int errnum){
...
bool ModbusTCP::read_coils(int addr, int nb, uint8_t *dest){
...
/*The result of reading is stored in dest array as unsigned bytes (8 bits) set to TRUE or FALSE.*/
bool ModbusTCP::read_discrete_inputs(int addr, int nb, uint8_t *dest){
...
bool ModbusTCP::read_holding_registers(int addr, int nb, uint16_t  *dest){
...
bool ModbusTCP::read_input_registers(int addr, int nb, uint16_t *dest){
...
bool ModbusTCP::getConnected(){
    return bConnected;
}
/**The status must be set to TRUE or FALSE.*/
bool ModbusTCP::write_coil(int addr, int status)
{
...
bool ModbusTCP::write_coils(int addr, int nb, const uint8_t *src){
...
bool ModbusTCP::write_holding_register(int addr, const uint16_t value){
...
bool ModbusTCP::write_holding_registers(int addr, int nb, const uint16_t *src){
...
bool ModbusTCP::write_and_read_holding_registers(int write_addr, int write_nb, const uint16_t *src,
